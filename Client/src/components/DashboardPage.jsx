...
import { getAll, add as addToStore, remove as removeFromStore } from '../lib/demoStore';
...
const DashboardPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = getAll('transaction_info');
        ...
        // Stock portfolio from the on-device demo store
        setStocks(getAll('stocks'));
  ...
  const handleAddStock = async (e) => {
    ...
    try {
      ...
      // Save the new stock to the on-device demo store
      const saved = addToStore('stocks', newStock);
      setStocks(prevStocks => [...prevStocks, saved]);
  ...
  const handleDeleteStock = async (id) => {
    ...
    if (confirmDelete) {
      try {
        // Delete stock from the on-device demo store
        removeFromStore('stocks', id);
  ...
  return (
    <div className="bg-cream min-h-screen text-ink p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-surface p-6 rounded shadow-md">
          ...
          {error ? (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr>
                  <th className="border-b border-surface-border p-2">Category</th>
                  <th className="border-b border-surface-border p-2">Total Amount</th>
                </tr>
              </thead>
              <tbody>
                {categoryExpenses.map((category, index) => (
                  <tr key={index} className="hover:bg-cream-deep">
                    <td className="border-b border-surface-border p-2 text-yellow-400">{category.name}</td>
                    <td className="border-b border-surface-border p-2">${category.value.toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-surface p-6 rounded shadow-md">
          ...
          <form onSubmit={handleAddStock} className="mb-4">
            <input
              ...
              className="p-2 rounded border border-surface-border bg-cream-deep mb-2 w-full"
              required
            />
            <input
              type="number"
            ...
            <input
              ...
              className="p-2 rounded border border-surface-border bg-cream-deep mb-4 w-full"
              required
            />
            <button type="submit" className="bg-accent hover:bg-accent-btn text-ink font-bold py-2 px-4 rounded">
              Add Stock
            </button>
          </form>
          {message && <p className="text-sage">{message}</p>}
          <table className="w-full text-left border-collapse mt-4">
            <thead>
              <tr>
                <th className="border-b border-surface-border p-2">Symbol</th>
                <th className="border-b border-surface-border p-2">Amount Spent</th>
                <th className="border-b border-surface-border p-2">Purchase Price</th>
                <th className="border-b border-surface-border p-2">Current Value</th>
                <th className="border-b border-surface-border p-2">Gains (%)</th>
                <th className="border-b border-surface-border p-2">Actions</th> {/* Added Actions column */}
              </tr>
            </thead>
            <tbody>
              {stocks.map((stock) => (
                <tr key={stock.id} className="hover:bg-cream-deep">
                  <td className="border-b border-surface-border p-2 text-yellow-400">{stock.symbol}</td>
                  <td className="border-b border-surface-border p-2">${stock.amountSpent.toFixed(2)}</td>
                  <td className="border-b border-surface-border p-2">${stock.purchasePrice.toFixed(2)}</td>
                  <td className="border-b border-surface-border p-2">${stock.currentValue.toFixed(2)}</td>
                  <td className="border-b border-surface-border p-2">{stock.gain.toFixed(2)}%</td>
                  <td className="border-b border-surface-border p-2">
                    <button
                      onClick={() => handleDeleteStock(stock.id)} // Call delete function
                      className="text-blush hover:text-red-600"
                    >
                      Delete
                    </button>
                  </td>
      ...
      <div className="bg-surface p-6 rounded shadow-md mt-6 w-full">
      ...
      {selectedCategory && (
        <div className="mt-8 bg-surface p-6 rounded shadow-md">
          ...
          {selectedCategory.transactions.length > 0 ? (
            <table className="w-full text-left border-collapse">
              <thead>
                <tr>
                  <th className="border-b border-surface-border p-2">Date & Time</th>
                  <th className="border-b border-surface-border p-2">Amount</th>
                  <th className="border-b border-surface-border p-2">Note</th>
                </tr>
              </thead>
              <tbody>
                {selectedCategory.transactions.map((transaction, index) => (
                  <tr key={index} className="hover:bg-cream-deep">
                    ...
                    <td className="border-b border-surface-border p-2">${parseFloat(transaction.amount).toFixed(2)}</td>
                    <td className="border-b border-surface-border p-2">{transaction.note}</td>
                  </tr>
                ))}
              </tbody>
            </table>
